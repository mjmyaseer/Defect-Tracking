...
class Issue extends \Eloquent
{
    ...
    public function update_cycle($issue_id)
    {
        $id = Issue::find($issue_id);
        $cycle = $id->id;
//        dd($id->id);
        $update = array(
            'cycles' => $cycle + 1
        ...
        Issue::find($issue_id)->fill($update)->save();


        $message = 'Hi';
        $to = 'mjmyaseer@gmail.com';
        $subject = 'Test Subject';
        \Mail::send_email($message, $to, $subject);
        return array(
            'success' => true
        );
    }
...
//        dd($project->id);
    ...
    public static function load_approval()
    {
        return $count = \DB::table('projects_issues')
            ->join('projects', 'projects.id', '=', 'projects_issues.project_id')
            ->where('projects.status', '=', 1)
    ...
    public function set_approval()
    {
        ...
        $projects_issues = new static;

        return $projects_issues->fill($insert)->save();
